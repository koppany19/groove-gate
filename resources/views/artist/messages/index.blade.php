        ...
        <div class="max-w-[1100px] mx-auto">
            ...
            @if($conversations->isEmpty())
                <div class="empty-state">
                    ...
                    <p class="text-(--color-muted) text-sm">Booking requests will appear here as conversations.</p>
                </div>
            @else
                <div class="space-y-3">
                    @foreach($conversations as $conversation)
                        <a href="{{ route('artist.messages.show', $conversation) }}" class="conversation-card">
                            <div class="flex items-center gap-4">

                                <div class="conversation-avatar">
                                    @if($conversation->organiser->avatar)
                                        <img src="{{ str_starts_with($conversation->organiser->avatar, 'http') ? $conversation->organiser->avatar : Storage::url($conversation->organiser->avatar) }}"
                                             class="w-full h-full object-cover">
                                    @else
                                        <span class="text-sm font-black text-(--color-artist)">
                                            {{ strtoupper(substr($conversation->organiser->name, 0, 1)) }}
                                        </span>
                                    @endif
                                </div>
